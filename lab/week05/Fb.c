...
void FbFriendRecs1(Fb fb, char *name) {
    ...
    //Start the assumption from a big number
    int mutual = MAX_PEOPLE;
    //Get 'name''s id
    int id = nameToId(fb, name);
    //Keep finding people that can be recommended
    while (mutual != 0) {
        //Looping from everyone
        for(int i = 0; fb->names[i] != NULL; i++) {
            //Check if 'names[i]' is not name him or her self and 'name''s friend
            if (!fb->friends[id][i] && id != i) {
                //Get numbers of mutual friends of 'names[i]' and 'name'
                int mfriends = ListSize(FbMutualFriends(fb, fb->names[i], name));
                //Check if 'names[i]' has currently most of mutual friends
                if (mfriends == mutual) {
                    printf("\t%-20s%4d mutual friends\n", fb->names[i], mfriends);
                }
            }
        }
        //decrement checked maximum numbers of mutual friends after recommended people with maximum numbers of mutual friends
        mutual --;
    }
}
